...
import numpy as np
import pandas as pd
from sys import argvimport argparse
...
fig = plt.figure()
ax = Axes3D(fig)
surf = ax.plot_trisurf(df.lm_alpha.values, df.lm_beta.values, df.wer.values, cmap=cm.jet, linewidth=0.1)
fig.colorbar(surf, shrink=0.5, aspect=5)
plt.savefig(args.json_path.replace('.csv','.svg'))
